...
enableSelection(input = null) {
  ...
  for (let i = 0; i < trArray.length; ++i) {
    trArray[i].addEventListener('click', function () {
      ...
      const rowValues = Array.from(trPrev.children).map(td => {
        return td.innerText;
      });
      ...
      const selected = {
        ...
        class: rowValues[1],
        price: rowValues[2].replaceAll('₱', ''),
        quantity: rowValues[3].replaceAll('x', ''),
        image: rowValues[4] // This will be the image URL from the database
      };
